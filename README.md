...
└── kaggle_execution.ipynb     staged end-to-end run (RUN_STAGE), restart-safe artifact reuse
...
`kaggle_execution.ipynb` provides a staged end-to-end execution workflow with
restart-safe artifact reuse: set `RUN_STAGE` (`data` / `baseline` / `uplift`
/ `causal_forest` / `report` / `all`) and Run All. Each stage loads a cached
artifact instead of recomputing it whenever one already matches the current
config, so a kernel restart between stages does not retrain anything. The
`01`–`04` notebooks below remain the way to read the reasoning behind each
step.
...
`kaggle_execution.ipynb`. Runtime outputs are gitignored.
