...
│                                    (denser sensitivity/robustness check, same
│                                    row partition -- see Methodology notes)
└── reports/
    └── comparative_analysis_report.ipynb   reader-facing synthesis across both
                                             outcomes -- loads existing artifacts
                                             only, trains/fits nothing; this is the
                                             notebook to read for the headline result
...
the notebook to read first for the actual result.
...
4. **Run `notebooks/kaggle_execution.ipynb`:** set `OUTCOME`
   (`"conversion"` primary, `"visit"` sensitivity check) and `RUN_STAGE`,
   then Run All.
