...
def test_predict_rejects_mismatched_columns() -> None:
    ...
    Y = 0.5 * X["f0"].to_numpy() + T * EFFECT_MAGNITUDE + noise
    model = _fit(X, T, Y)
    bad = X.rename(columns={"f0": "f0_renamed"})
...
def test_synthetic_gate_zero_effect() -> None:
    ...
    Y = 0.5 * X["f0"].to_numpy() + noise  # Y independent of T
    model = _fit(X, T, Y)
    tau_hat = predict_tau(model, X)
...
def test_synthetic_gate_heterogeneous_effect() -> None:
    ...
    regime = (X["f0"].to_numpy() > 0).astype(np.float64)
    true_tau = EFFECT_MAGNITUDE * regime
    Y = 0.5 * X["f1"].to_numpy() + T * true_tau + noise
    model = _fit(X, T, Y)
    tau_hat = predict_tau(model, X)
...
def test_aggregate_jacobian_support_passes_despite_per_leaf_degeneracy() -> None:
    ...
    Y = 0.5 * X["f0"].to_numpy() + T_imbalanced * EFFECT_MAGNITUDE + noise
